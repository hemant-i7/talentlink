...
export default function ManagerDashboard() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (type === "checkbox") {
      setBrandDetails((prev) => ({ ...prev, [name]: (e.target as HTMLInputElement).checked }))
    } else {
  ...
  const handleAddBrand = async () => {
    if (!brandDetails.name || !brandDetails.description || !brandDetails.moneyOffered || !brandDetails.imageUrl) {
      ...
      <Tabs defaultValue="current" className="w-full">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredBrands.map((brand) => (
                <Card key={brand._id} className="bg-gray-800 border-gray-700 hover:bg-gray-700 transition-colors">
                  <CardHeader className="flex flex-row items-center gap-4">
                    <img src={brand.imageUrl} alt={brand.name} className="w-16 h-16 rounded-full object-cover" />
                    <CardTitle>{brand.name}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-gray-400 mb-2">{brand.description}</p>
                    <p className="text-sm font-bold text-white">Sponsorship: {brand.moneyOffered}</p>
                    <p className="text-sm font-medium">
                      Status:{" "}
                      <span className={brand.sponsorshipAvailable ? "text-green-500" : "text-red-500"}>
        ...
        <TabsContent value="add">
          <Card className="bg-gray-800 border-gray-700">
                ...
                <label className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setBrandDetails((prev) => ({ ...prev, sponsorshipAvailable: !!checked }))
                    }
                  />
            ...
            <CardFooter>
              <Button onClick={handleAddBrand} className="w-full bg-white hover:bg-white/20">
                Add Brand Deal
              </Button>
            </CardFooter>
          </Card>
